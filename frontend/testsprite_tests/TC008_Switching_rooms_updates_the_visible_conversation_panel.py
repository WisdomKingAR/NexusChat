...
async def run_test():
    ...
    try:
        ...
        # -> Navigate (explicit) to /login (http://localhost:4173/login) as required by the test steps.
        ...
        # -> Click the 'Sign In' button (index 65) again and wait up to 5 seconds for the app to redirect/render the main chat UI (rooms/sidebar). After wait, check for room elements.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/div/form/button').nth(0)
        await asyncio.sleep(3); await elem.click()

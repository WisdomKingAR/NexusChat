...
async def run_test():
    ...
    try:
        ...
        # -> Navigate to /login (explicit test step) so the login page can be loaded and interactive elements inspected.
        ...
        # -> Click the 'Test Room for Messaging' room in the sidebar (interactive element index 306) to open the room view.
        frame = context.pages[-1]
        # Click element
        elem = frame.locator('xpath=/html/body/div/div/div/aside/div/div/div[2]/button[3]').nth(0)
        await asyncio.sleep(3); await elem.click()
        
        # -> Type the message 'E2E message via send icon' into the message input (index 368) and then click the send icon/button (index 369).
        frame = context.pages[-1]
        # Input text
        elem = frame.locator('xpath=/html/body/div/div/div/main/div/form/div/input').nth(0)
        await asyncio.sleep(3); await elem.fill('E2E message via send icon')
